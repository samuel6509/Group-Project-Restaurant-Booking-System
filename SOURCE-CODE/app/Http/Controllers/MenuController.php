...
class MenuController extends Controller
{
    ...
    public function getMenuItems(Request $request, $menuType)
    {
        ...
        $items = $menu->where('itemType', $itemType)->get()->map(function ($item) {
            $item->itemImageURL = asset($item->itemImageURL);
            return $item;
        });
    ...
    public function menuItems($menuType = 'morning_menu')
    {
        ...
        $items = $menu->orderBy('itemType')->paginate(10)->through(function ($item) {
            $item->itemImageURL = asset($item->itemImageURL);
            return $item;
        });
